...
export class ProductService {
  ...
  async createProduct(data: any) {
    ...
    const sanitizedData = {
      ...
      slug: data.slug || (data.name ? data.name.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '') : `prod-${Date.now()}`),
  ...
  async updateProduct(id: string, data: any) {
    ...
    if (data.original_price !== undefined) sanitizedData.original_price = Number(data.original_price);
    ...
    if (data.stock_quantity !== undefined) sanitizedData.stock_quantity = Number(data.stock_quantity);
    if (data.status) sanitizedData.status = data.status;
    if (data.images) sanitizedData.images = Array.isArray(data.images) ? data.images : [data.images];
