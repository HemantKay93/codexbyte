...
async function seed() {
  ...
  if (authError) {
    ...
    console.log('Attempting to proceed as anonymous (this only works if RLS is disabled on user_profiles)...');
  } else {
    console.log('Auth successful. User ID:', authData.user.id);
  }

  const admins = [
    { email: 'admin@byteevolvr.com', id: authData?.user?.id || '6d6b5509-8a60-4123-92c1-55b52942d144' },
  ...
  for (const admin of admins) {
    ...
    if (upsertError) {
      ...
      console.log(`Could not directly insert ${admin.email}. They will be automatically added when they first log in.`);
    }
  }
