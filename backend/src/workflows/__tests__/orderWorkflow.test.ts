...
describe('OrderWorkflow', () => {
  ...
  it('should successfully process a checkout and dispatch analytics', async () => {
    ...
    const result = await OrderWorkflow.processCheckout('user_1', { items: mockItems, warehouseId: 'wh_1', paymentMethod: 'cash' });
    ...
    expect(mockCreateOrder).toHaveBeenCalledWith('user_1', { items: mockItems, warehouseId: 'wh_1', paymentMethod: 'cash' }, undefined);
  ...
  it('should rollback inventory reservations if order creation fails', async () => {
    ...
    await expect(
      OrderWorkflow.processCheckout('user_1', { items: mockItems, warehouseId: 'wh_1', paymentMethod: 'cash' })
    ...
    expect(InventoryService.releaseReservation).toHaveBeenCalledWith(expect.objectContaining({
      productId: 'p1',
      warehouseId: 'wh_1',
      quantity: 2,
      reservationId: 'res_1'
