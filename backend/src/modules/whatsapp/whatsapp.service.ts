...
export class WhatsAppService {
  ...
  static async enqueueMessage(to: string, payload: Omit<WhatsAppMessagePayload, 'to'>, priority: number = 0) {
    ...
    const isServerlessOrProd = !!process.env.VERCEL || !!process.env.RENDER || process.env.NODE_ENV === 'production';

    if (isServerlessOrProd) {
      logger.info(`[WhatsAppService] Running in serverless/production environment. Processing synchronously to guarantee delivery.`);
      try {
        const { WhatsAppProviderFactory } = await import('../marketing/providers/whatsapp/whatsappProviderFactory.js');
