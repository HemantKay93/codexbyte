...
export class BankingService {
  ...
  static async createBankAccount(payload: any) {
    ...
    if (account.type !== 'Asset') throw new Error('Bank accounts must map to an Asset account in the CoA');
  ...
  static async recordTransaction(bankAccountId: string, payload: any) {
    ...
    if (!payload.offset_account_id) throw new Error('Must provide an offset account ID to balance the transaction');

    // 3. Post Journal
    const isDeposit = payload.transaction_type === 'deposit';
    let debitAccount = isDeposit ? bankAcc.account_id : payload.offset_account_id;
    let creditAccount = isDeposit ? payload.offset_account_id : bankAcc.account_id;
    ...
    const newBalance = isDeposit ? Number(bankAcc.current_balance) + amount : Number(bankAcc.current_balance) - amount;
    await admin
      .from('bank_accounts')
