...
export function OrderActivityLogs({ orderId }: { orderId: string }) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'packed': return <Package className="h-4 w-4 text-indigo-500" />;
      case 'confirmed': return <PlusCircle className="h-4 w-4 text-primary" />;
      case 'cancelled': return <X className="h-4 w-4 text-error" />;
      case 'returned': return <RotateCcw className="h-4 w-4 text-warning" />;
      case 'refunded': return <DollarSign className="h-4 w-4 text-success" />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'shipped': return 'border-info bg-info/10';
      case 'packed': return 'border-indigo-500 bg-indigo-50';
      case 'confirmed': return 'border-primary bg-primary/10';
      case 'cancelled': return 'border-error bg-error/10';
      case 'returned': return 'border-warning bg-warning/10';
      case 'refunded': return 'border-success bg-success/10';
      default: return 'border-outline-variant bg-surface-container';
    }
  };
        ...
        <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-outline-variant">
          {logs.map((log) => (
            <div key={log.id} className="relative flex items-start gap-6">
              <div className={`absolute left-0 h-10 w-10 rounded-full border flex items-center justify-center z-10 ${getStatusColor(log.status)}`}>
