                    ...
                    <TableCell>
                      <div className="space-y-1">
                        {entry.lines?.map((line: any) => (
                          <div key={line.id} className="flex justify-between text-xs w-64 border-b border-outline-variant/30 pb-1 last:border-0">
                            <span className="truncate pr-2">{line.account?.name || 'Unknown Account'}</span>
                            <span className="font-medium shrink-0">
                              {Number(line.debit_amount) > 0 
                                ? <span className="text-on-surface">DR ₹{Number(line.debit_amount).toLocaleString()}</span>
                                : <span className="text-on-surface-variant">CR ₹{Number(line.credit_amount).toLocaleString()}</span>}
                            </span>
                          </div>
                        ))}
                      </div>
            ...
            <form onSubmit={handleCreateEntry} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g. Office Rent Payment" required />
                </div>
              </div>
                  ...
                  <table className="w-full text-sm text-left">
                      ...
                      {lines.map((line, idx) => (
                        <tr key={idx}>
                          ...
                          <td className="px-4 py-2 text-center">
                            {lines.length > 2 && (
                              <button type="button" onClick={() => removeLine(idx)} className="text-error hover:bg-error/10 p-1 rounded">
                    ...
                    <tfoot className="bg-surface-container font-medium border-t border-outline-variant">
                      <tr>
                        ...
                        <td className={`px-4 py-3 ${totalDebit === totalCredit ? 'text-success' : 'text-error'}`}>₹{totalDebit.toLocaleString()}</td>
                        <td className={`px-4 py-3 ${totalDebit === totalCredit ? 'text-success' : 'text-error'}`}>₹{totalCredit.toLocaleString()}</td>
                        <td></td>
                      </tr>
                    </tfoot>
                  </table>
