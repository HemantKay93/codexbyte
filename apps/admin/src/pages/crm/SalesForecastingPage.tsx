...
export function SalesForecastingPage() {
  ...
  const percentToQuota = (totalCommitted / totalQuota) * 100;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <Card className="p-6 border-l-4 border-l-primary">
          ...
          <h3 className="text-2xl font-bold text-primary">₹{totalCommitted.toLocaleString()}</h3>
        </Card>
        <Card className="p-6">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-sm font-bold text-on-surface">{percentToQuota.toFixed(1)}%</span>
          </div>
        </Card>
        <Card className="p-6 flex items-center justify-between">
          <div>
            <p className="text-sm text-on-surface-variant mb-1">Projected Best Case</p>
            <h3 className="text-2xl font-bold text-success">₹22,00,000</h3>
          </div>
      ...
      <Card className="overflow-hidden">
        ...
        <Table>
          ...
          <TableBody>
            {forecasts.map((f, i) => {
              const attainment = (f.committed / f.quota) * 100;
              return (
                <TableRow key={i}>
