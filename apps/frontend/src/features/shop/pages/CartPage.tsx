...
import { ShoppingCart, Minus, Plus, Trash2, ArrowRight, ShieldCheck, Lock, ArrowLeft } from 'lucide-react';

export function CartPage() {
  const navigate = useNavigate();
  const { items, removeItem, updateQuantity, totalAmount, addItem } = useCartStore();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-stitch-gutter items-start">
        ...
        <div className="lg:col-span-8 space-y-stitch-gutter">
          <div className="flex items-baseline justify-between mb-4">
            <h2 className="font-stitch-headline-lg text-stitch-headline-lg text-white">YOUR_CRATE</h2>
            <span className="font-stitch-label-sm text-stitch-label-sm text-stitch-outline uppercase">[ {items.length < 10 ? `0${items.length}` : items.length} Items ]</span>
          </div>
          
          {items.map((item) => (
            <div key={item.id} className="stitch-glass-panel rounded-xl p-stitch-base md:p-6 flex flex-col md:flex-row gap-6 relative overflow-hidden group">
              ...
              <div className="flex-grow flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start">
                    <h3 className="font-stitch-headline-lg-mobile text-stitch-headline-lg-mobile text-stitch-primary">{item.name}</h3>
                    <p className="font-stitch-label-sm text-stitch-headline-lg-mobile text-white">{currencySymbol}{(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                  <p className="font-stitch-body-md text-stitch-on-surface-variant mt-2 line-clamp-2">Premium Tech Gear</p>
                  <div className="mt-4 flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-stitch-secondary animate-pulse" style={{ animationDuration: '2s' }}></span>
                    <span className="font-stitch-label-sm text-stitch-secondary uppercase">In Stock &amp; Ready for Dispatch</span>
                  </div>
                </div>
                <div className="flex items-center justify-between mt-6">
                  <div className="flex items-center border border-stitch-outline-variant/30 rounded-full bg-stitch-surface-container-low px-2 py-1">
                    ...
                    <span className="px-4 font-stitch-label-sm text-white">{item.quantity < 10 ? `0${item.quantity}` : item.quantity}</span>
                    <button 
          ...
          {relatedProducts.length > 0 && (
            <div className="pt-8">
              <h3 className="font-stitch-label-sm text-stitch-label-sm text-stitch-on-surface-variant mb-4 tracking-widest uppercase">SYNERGIZE_YOUR_SETUP</h3>
              <div className="flex gap-stitch-gutter overflow-x-auto pb-4 stitch-no-scrollbar">
                {relatedProducts.map(product => (
                  <div key={product.id} className="min-w-[200px] stitch-glass-panel rounded-xl p-4 group flex flex-col justify-between">
                    <Link to={`/shop/product/${product.id}`} className="block h-32 bg-stitch-surface rounded mb-3 overflow-hidden cursor-pointer">
                      <img className="w-full h-full object-cover group-hover:scale-105 transition-transform" src={product.image_url || 'https://via.placeholder.com/200'} alt={product.name} />
                    </Link>
                    <div>
                      <Link to={`/shop/product/${product.id}`} className="font-stitch-label-sm text-white text-xs truncate block hover:text-stitch-primary transition-colors">{product.name}</Link>
                      <p className="font-stitch-label-sm text-stitch-secondary text-xs mt-1 mb-3">{currencySymbol}{product.price.toFixed(2)}</p>
                      <button 
        ...
        <div className="lg:col-span-4 lg:sticky lg:top-24 space-y-stitch-gutter">
          <div className="stitch-glass-panel rounded-xl p-6 border-stitch-primary/20 bg-stitch-surface-container-high/60">
            <h3 className="font-stitch-headline-lg-mobile text-stitch-headline-lg-mobile text-white mb-6 uppercase">ORDER_MANIFEST</h3>
            ...
            <div className="border-t border-stitch-outline-variant/30 pt-6 mb-8">
              <div className="flex justify-between items-baseline">
                ...
                <span className="font-stitch-display-lg-mobile text-stitch-display-lg-mobile text-stitch-primary">{currencySymbol}{(total * 1.1).toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="mb-8">
              <div className="relative flex items-center bg-stitch-surface-container-lowest border border-stitch-outline-variant/30 rounded-lg p-1 transition-all focus-within:border-stitch-primary focus-within:shadow-[0_0_15px_rgba(173,198,255,0.3)]">
                <input 
                  ...
                  type="text" 
                />
                <button className="bg-stitch-surface-variant text-white font-stitch-label-sm text-stitch-label-sm px-4 py-2 rounded-md hover:bg-stitch-outline-variant transition-colors">APPLY</button>
              </div>
            </div>
            ...
            <div className="mt-8 flex justify-center items-center gap-6">
              <div className="flex flex-col items-center gap-1 opacity-60">
                ...
                <span className="font-stitch-label-sm text-[10px] uppercase text-white">Secure Tx</span>
              </div>
              <div className="w-px h-8 bg-stitch-outline-variant/30"></div>
              <div className="flex flex-col items-center gap-1 opacity-60">
                <Lock className="w-6 h-6 text-stitch-secondary" />
                <span className="font-stitch-label-sm text-[10px] uppercase text-white">Encrypted</span>
              </div>
            </div>
          </div>
          
          {/* Back to Shopping */}
          <Link to="/shop" className="flex items-center justify-center gap-2 font-stitch-label-sm text-stitch-outline hover:text-stitch-primary transition-colors group uppercase tracking-widest mt-4">
