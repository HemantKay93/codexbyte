...
import { Loader2, LayoutGrid, List, SlidersHorizontal, ChevronDown, Heart, Star, Plus, Check } from 'lucide-react';
  ...
  return (
    <div className="flex w-full">
      ...
      <aside className="hidden md:block sticky top-0 left-0 w-80 h-screen pt-24 pb-8 border-r border-stitch-outline-variant/30 bg-stitch-surface/80 backdrop-blur-xl px-8 overflow-y-auto stitch-no-scrollbar">
        <h2 className="font-stitch-headline-lg text-stitch-headline-lg-mobile uppercase text-stitch-primary mb-8">Filters</h2>
        <div className="space-y-10">
          <section>
            <h3 className="font-stitch-label-sm text-stitch-label-sm text-stitch-outline uppercase mb-4">Brand</h3>
            <div className="space-y-4">
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="group-hover:text-stitch-primary text-stitch-on-surface-variant transition-colors">Logitech G</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="group-hover:text-stitch-primary text-stitch-on-surface-variant transition-colors">Corsair</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className="w-5 h-5 rounded border border-stitch-outline-variant bg-stitch-surface-container-high flex items-center justify-center group-hover:border-stitch-primary transition-colors">
                </div>
                <span className="group-hover:text-stitch-primary text-stitch-on-surface-variant transition-colors">HyperX</span>
              </label>
            </div>
          </section>
          
          <section>
            <h3 className="font-stitch-label-sm text-stitch-label-sm text-stitch-outline uppercase mb-4">Price Range</h3>
            ...
            <div className="flex justify-between mt-3 font-stitch-label-sm text-stitch-label-sm text-stitch-outline">
              <span className="bg-stitch-surface-container-high px-2 py-1 rounded">{currencySymbol}49</span>
              <span className="bg-stitch-surface-container-high px-2 py-1 rounded">{currencySymbol}499</span>
            </div>
          </section>
        </div>
      </aside>
      ...
      <main className="pt-24 pb-20 px-stitch-container-padding-mobile md:px-8 min-h-screen w-full">
        ...
        {loading ? (
          ...
          <div className={`grid ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-stitch-gutter' : 'grid-cols-1 max-w-3xl mx-auto gap-stitch-gutter'} transition-all duration-500`}>
            {products.map((product) => (
              <div key={product.id} className="stitch-glass-panel rounded-2xl p-4 transition-all duration-300 group flex flex-col hover:border-stitch-secondary/50">
                <Link to={`/shop/product/${product.slug || product.id}`} className="relative aspect-square rounded-xl overflow-hidden mb-4 bg-stitch-surface-container-high block group/image">
                  <img src={product.image_url || 'https://via.placeholder.com/150'} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 group-hover/image:scale-110 opacity-90 group-hover/image:opacity-100" />
                ...
                <div className="flex-grow">
                  <div className="flex items-center gap-1 text-stitch-secondary mb-2">
                    ...
                    <span className="text-stitch-outline font-normal">({product.reviews?.length || 0})</span>
                  </div>
                  <Link to={`/shop/product/${product.slug || product.id}`}>
                    <h3 className="font-stitch-headline-lg-mobile text-white mb-2 group-hover:text-stitch-primary transition-colors">{product.name}</h3>
                  </Link>
                  <p className="text-stitch-outline text-stitch-label-sm mb-4 line-clamp-1">{product.description}</p>
                  <div className="mb-4">
                    <div className="flex items-baseline gap-2">
                      <span className="text-stitch-headline-lg-mobile text-stitch-primary">{currencySymbol}{product.price.toFixed(2)}</span>
                      {(product.original_price && product.original_price > product.price) && (
                        <span className="text-stitch-outline line-through text-stitch-label-sm">{currencySymbol}{product.original_price.toFixed(2)}</span>
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex items-center justify-between pt-4 border-t border-stitch-outline-variant/20">
                  <label className="flex items-center gap-2 cursor-pointer group/comp">
                    ...
                    <span className="text-stitch-label-sm text-stitch-outline group-hover/comp:text-white transition-colors">Compare</span>
                  </label>
                  <button 
