...
export function ShopPage() {
  ...
  const renderGlassCard = (card: any, index: number) => {
    return (
      <Card key={index} className="flex flex-col bg-[#070D1A]/80 backdrop-blur-xl border border-white/10 h-[400px] hover:border-accent/50 transition-all duration-300 group overflow-hidden">
        <div className="flex-1 p-6 flex flex-col">
          <h3 className="text-xl font-display font-bold text-white mb-4 leading-tight group-hover:text-accent transition-colors">{card.title}</h3>
          ...
          {card.type === 'signin' ? (
            <div className="flex-1 flex flex-col justify-center items-center text-center">
              ...
              <p className="text-brand-muted mb-6">Unlock exclusive deals and track your orders seamlessly.</p>
              <Link to="/shop/login" className="w-full">
                <Button variant="primary" className="w-full">Sign In Securely</Button>
              </Link>
            </div>
          ) : card.type === 'single' ? (
            ...
            <div className="flex-1 grid grid-cols-2 gap-3 mb-4">
              {card.items.map((item: any, i: number) => (
                <div key={i} className="relative rounded-lg overflow-hidden group/item cursor-pointer">
                  <img src={item.image} alt={item.name} className="w-full h-24 object-cover transition-transform duration-300 group-hover/item:scale-110 opacity-80 group-hover/item:opacity-100" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-2">
                    <span className="text-xs font-semibold text-white drop-shadow-md">{item.name}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {card.linkText && (
            <Link to="/shop" className="text-sm font-bold text-accent flex items-center gap-1 hover:gap-2 transition-all mt-auto">
      ...
      <div className="mb-16">
        ...
        <div className="flex overflow-x-auto pb-6 gap-6 scrollbar-hide snap-x px-2">
          {products.map(product => (
            <div key={product.id} className="min-w-[280px] max-w-[280px] snap-start" onClick={() => navigate(`/shop/product/${product.id}`)}>
              <Card className="h-full bg-[#070D1A] border-white/5 hover:border-accent/30 transition-all duration-300 cursor-pointer overflow-hidden p-0 group">
                <div className="relative h-[220px] bg-white/5 overflow-hidden p-6 flex items-center justify-center">
                  <div className="absolute top-3 left-3 z-10">
                    <Badge variant="primary" className="bg-accent/20 text-accent border-accent/20 backdrop-blur-md">Hot Deal</Badge>
                  </div>
                  <img src={product.image_url || product.imageUrl} alt={product.name} className="max-h-full max-w-full object-contain transition-transform duration-500 group-hover:scale-110 drop-shadow-2xl" />
                </div>
                <div className="p-5 flex flex-col gap-2">
                  <div className="text-xs font-semibold text-brand-muted uppercase tracking-wider">{product.brand}</div>
                  <h4 className="text-lg font-bold text-white line-clamp-2 leading-tight group-hover:text-accent transition-colors">{product.name}</h4>
                  <div className="mt-4 flex items-end gap-3">
                    <span className="text-2xl font-bold text-white">{formatPrice(Number(product.price))}</span>
                    {product.original_price && product.original_price > product.price && (
                      <span className="text-sm text-white/40 line-through mb-1">{formatPrice(Number(product.original_price))}</span>
                    )}
                  </div>
                </div>
              </Card>
    ...
    <div className="min-h-screen bg-[#04080F] text-white selection:bg-accent/30">
      <main className="relative mx-auto pb-20">
          ...
          <div className="relative z-10 max-w-[1500px] mx-auto px-6 pt-32 md:pt-40">
            <div className="max-w-2xl">
              ...
              <h2 className="text-5xl md:text-7xl font-display font-bold leading-tight mb-6">
                Elevate Your <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-purple-500">Digital Realm</span>
              </h2>
              <p className="text-lg md:text-xl text-white/70 mb-10 max-w-xl leading-relaxed">
                Discover enterprise-grade workstations, immersive displays, and ultra-fast networking gear curated for professionals.
              </p>
              <div className="flex gap-4">
                <Button variant="primary" size="lg" className="rounded-xl px-8 shadow-[0_0_20px_rgba(59,130,246,0.3)]">
                  Explore Catalog
                </Button>
                <Button variant="ghost" size="lg" className="rounded-xl px-8 border border-white/20 hover:bg-white/5">
                  View Deals
                </Button>
              </div>
            </div>
        ...
        <div className="relative z-10 max-w-[1500px] mx-auto px-6">
          ...
          {loading ? (
            <div className="flex justify-center items-center py-20"><Loader2 className="animate-spin text-accent" /></div>
          ) : products.length > 0 ? (
            ...
            <div className="text-center py-20 bg-[#070D1A] border border-white/10 rounded-2xl mb-20">
              ...
              <p className="text-brand-muted">Please add products via the Admin panel or run your database migrations.</p>
            </div>
          )}
          ...
          {!loading && products.length > 0 && renderPremiumCarousel(
