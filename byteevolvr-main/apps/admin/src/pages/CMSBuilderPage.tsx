...
export function CMSBuilderPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const updates = Object.entries(cmsData).map(([key, content]) => ({
        page_slug: 'home',
        section_key: key,
        content: content,
    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col -m-6">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 bg-surface-dim overflow-y-auto p-8 flex justify-center">
          <div className={`bg-white shadow-xl min-h-full border border-outline-variant transition-all duration-300 relative ${device === 'mobile' ? 'w-[375px]' : 'w-full max-w-5xl'}`}>
            
            {/* Navbar Preview */}
            <div className={`p-4 border-b flex items-center justify-between bg-white ${selectedSection === 'navbar' ? 'ring-2 ring-primary ring-inset' : ''}`}>
              <div className="font-bold text-xl text-primary">{cmsData.navbar.logoText || 'Logo'}</div>
            ...
            <div 
              ...
              style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url(${cmsData.hero.backgroundImage})` }}
            >
              <h1 className="text-4xl font-bold mb-4 z-10">{cmsData.hero.title || 'Main Title Goes Here'}</h1>
              <p className="max-w-2xl mb-8 opacity-90 z-10">{cmsData.hero.subtitle || 'Your subtitle or description.'}</p>
            ...
            {/* Contact Preview */}
            <div className={`p-12 border-t bg-white ${selectedSection === 'contact' ? 'ring-2 ring-primary ring-inset' : ''}`}>
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <h3 className="font-bold text-lg mb-4">Contact Us</h3>
                  <div className="space-y-2 text-sm text-gray-600">
                    <div className="flex items-center gap-2"><MapPin size={16} /> {cmsData.contact.address}</div>
                    <div className="flex items-center gap-2"><Phone size={16} /> {cmsData.contact.phone}</div>
                    <div className="flex items-center gap-2"><Mail size={16} /> {cmsData.contact.email}</div>
                  </div>
                </div>
                <div>
                  <h3 className="font-bold text-lg mb-4">Hours</h3>
                  <div className="flex items-center gap-2 text-sm text-gray-600"><Clock size={16} /> {cmsData.contact.workingHours}</div>
                </div>
              </div>
            </div>
        ...
        <div className="w-80 border-l border-outline-variant bg-surface-container-lowest overflow-y-auto shrink-0 p-6">
          <h2 className="text-lg font-semibold text-on-surface mb-6">Edit {selectedSection.charAt(0).toUpperCase() + selectedSection.slice(1)}</h2>
          ...
          <div className="space-y-6">
            {selectedSection === 'hero' && (
              <>
                ...
                <div>
                  ...
                  <Input value={cmsData.hero.buttonText} onChange={(e) => updateContent('hero', 'buttonText', e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Background Image URL</label>
                  <Input value={cmsData.hero.backgroundImage} onChange={(e) => updateContent('hero', 'backgroundImage', e.target.value)} />
                </div>
              </>
            )}
            ...
            {selectedSection === 'navbar' && (
              <>
                <div>
                  ...
                  <Input value={cmsData.navbar.logoText} onChange={(e) => updateContent('navbar', 'logoText', e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-3">Navigation Links</label>
                  <div className="space-y-4">
                    {cmsData.navbar.links?.map((link: any, i: number) => (
                      <div key={i} className="p-3 border border-outline rounded-lg bg-surface-container-lowest space-y-3 relative group">
                        <Input 
                          placeholder="Label" 
                          value={link.label} 
                          onChange={(e) => {
                        ...
                        <button 
                          onClick={() => {
                            const newLinks = cmsData.navbar.links.filter((_: any, idx: number) => idx !== i);
                            updateContent('navbar', 'links', newLinks);
                          }}
                    ...
                    <Button 
                      ...
                      fullWidth 
                      onClick={() => {
                        const newLinks = [...(cmsData.navbar.links || []), { label: 'New Link', href: '/' }];
                        updateContent('navbar', 'links', newLinks);
                      }}
                    >
                      + Add Nav Link
            ...
            {selectedSection === 'contact' && (
              <>
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Physical Address</label>
                  <textarea 
                ...
                <div>
                  ...
                  <Input value={cmsData.contact.phone} onChange={(e) => updateContent('contact', 'phone', e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Email Address</label>
                  <Input value={cmsData.contact.email} onChange={(e) => updateContent('contact', 'email', e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-on-surface mb-2">Working Hours</label>
                  <Input value={cmsData.contact.workingHours} onChange={(e) => updateContent('contact', 'workingHours', e.target.value)} />
                </div>
              </>
            )}
          </div>
