...
export function ReviewsPage() {
  ...
  const renderStars = (rating: number) => {
    return Array(5).fill(0).map((_, i) => (
      <Star key={i} className={`h-4 w-4 ${i < rating ? 'text-amber-400 fill-amber-400' : 'text-outline-variant'}`} />
    ));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className={stats.pending > 0 ? "bg-warning-container border-warning-container text-on-warning-container" : ""}>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-12">
                    ...
                    <span className="text-sm text-on-surface-variant mt-2 block">Loading reviews...</span>
                  </TableCell>
                </TableRow>
                ...
                <TableRow key={review.id} className="items-start">
                  <TableCell className="align-top pt-4">
                    <div className="font-medium text-on-surface">{review.user?.full_name || 'Anonymous'}</div>
                  </TableCell>
                  <TableCell className="align-top pt-4">
                    <div className="text-primary hover:underline cursor-pointer font-medium text-sm">{review.product?.name}</div>
                  </TableCell>
