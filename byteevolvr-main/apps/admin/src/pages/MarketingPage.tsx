...
import { Plus, Zap, Send, MousePointerClick, TrendingUp, MoreHorizontal, Calendar } from 'lucide-react';

const workflows = [
  { id: '1', name: 'Abandoned Cart Recovery', trigger: 'Cart idle for 2 hours', active: true, sent: 1245, conversion: '12.4%' },
  { id: '2', name: 'Welcome Series - New Users', trigger: 'On Sign up', active: true, sent: 8430, conversion: '45.2%' },
  { id: '3', name: 'Post-Purchase Review Request', trigger: '7 days after delivery', active: true, sent: 3210, conversion: '8.1%' },
  { id: '4', name: 'Win-back Campaign (90 days)', trigger: 'No purchase for 90 days', active: false, sent: 540, conversion: '2.3%' },
];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-body-sm text-on-surface-variant mt-1">Design and manage automated email & SMS workflows</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="bg-primary-container border-primary-container text-on-primary-container">
          <CardContent className="p-6 flex flex-col items-center justify-center text-center h-full">
            ...
            <Button variant="secondary" size="sm" className="w-full">Quick Start</Button>
          </CardContent>
        </Card>
      </div>
          ...
          <TableBody>
            {workflows.map(wf => (
              <TableRow key={wf.id}>
                ...
                <TableCell className="text-right text-on-surface-variant">{wf.sent.toLocaleString()}</TableCell>
                <TableCell className="text-right font-medium text-emerald-600">{wf.conversion}</TableCell>
                <TableCell>
      ...
      <h2 className="text-lg font-semibold text-on-surface mt-8 mb-4">Recent Campaign (Visualizer)</h2>
      <div className="bg-surface-container-low border border-outline-variant rounded-xl p-8 flex flex-col items-center">
        <div className="bg-surface border border-outline border-l-4 border-l-primary p-4 rounded shadow-sm w-64 text-center">
          <div className="text-xs font-bold text-primary mb-1 uppercase tracking-wider">Trigger</div>
        ...
        <div className="bg-surface border border-outline border-l-4 border-l-warning p-4 rounded shadow-sm w-64 text-center">
          <div className="text-xs font-bold text-warning mb-1 uppercase tracking-wider">Condition</div>
